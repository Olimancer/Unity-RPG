...
namespace Game.CameraNS {
    public class CameraRaycaster : MonoBehaviour {
        ...
        private Layer                   _lastLayerHit       = Layer.Default;

        public delegate void OnLayerChange(Layer newLayer); // When the cursor has moved over another layer
        public event OnLayerChange _notifyLayerChange;

        public delegate void OnPriorityLayerClicked(RaycastHit raycastHit, Layer layerHit); // Whenever the player click
        ...
        void NotifyLayerChangeIfNew(Layer currentLayer) {
            ...
            foreach (int layerInt in _layerPriorities) {
                foreach (RaycastHit hit in raycastHits) {
                    if (hit.collider.gameObject.layer == layerInt) {
                        NotifyLayerChangeIfNew((Layer)layerInt);
                        return hit;
                    }
                }
            }
